 ...
 for j in "${!partitions_array[@]}"; do
  ...
  gnuplot << EOF
      ...
      #  plot sorting_points u 7:8 w points ls count notitle
        plot "data_mfcompress-$NAF_synthetic_CSV-$SORTING_TYPE-${partitions_array[j]}_fasta_analysis.csv" u 9:8 title "Synthetic Data" with linespoints linestyle count , "data_mfcompress-$NAF_real_CSV-$SORTING_TYPE-${partitions_array[j]}_fasta_analysis.csv" u 9:8 title "Real Data" with linespoints linestyle count
       # count=count + 1
       # plot 
       # count=count + 1
